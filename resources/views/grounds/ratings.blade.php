...
@section('title', 'Ratings - ' . $ground->name)

@section('content')
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-8">
        <div class="flex items-center mb-4">
            <a href="{{ route('grounds.show', $ground) }}" class="text-indigo-600 hover:text-indigo-700 flex items-center">
                <i class="fas fa-arrow-left mr-2"></i> Back to Ground
            </a>
        </div>
        <h1 class="text-3xl font-bold text-gray-900 mb-2">{{ $ground->name }} - User Ratings</h1>
        <p class="text-gray-600">See what users think about this ground</p>
    </div>
    ...
    <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-2xl shadow-lg p-8 mb-8 text-white">
        <div class="flex flex-col md:flex-row items-center justify-between">
            <div class="text-center md:text-left mb-6 md:mb-0">
                <h2 class="text-2xl font-semibold mb-2">Overall Ground Rating</h2>
            ...
            @if($totalRatings > 0)
                <div class="md:w-64">
                    <h3 class="font-semibold mb-3 text-sm">Rating Distribution</h3>
                    @foreach([5, 4, 3, 2, 1] as $stars)
                        <div class="flex items-center mb-2">
                            <span class="text-sm w-12">{{ $stars }} <i class="fas fa-star text-yellow-300 text-xs"></i></span>
                            <div class="flex-1 mx-2 bg-white bg-opacity-30 rounded-full h-2">
                                <div 
                                    class="bg-white rounded-full h-2 transition-all" 
                                    style="width: {{ $totalRatings > 0 ? ($ratingDistribution[$stars] / $totalRatings * 100) : 0 }}%"
                                ></div>
                            </div>
                            <span class="text-sm w-8 text-right">{{ $ratingDistribution[$stars] }}</span>
                        </div>
                    @endforeach
                </div>
            @endif
    ...
    @if($userRating)
        <div class="bg-blue-50 border border-blue-200 rounded-xl p-6 mb-8">
            <h3 class="font-semibold text-blue-900 mb-4 flex items-center">
                <i class="fas fa-check-circle mr-2"></i> Your Rating for {{ $ground->name }}
            </h3>
            ...
            <div>
                <form 
                    action="{{ route('ground-ratings.destroy', [$ground, $userRating->id]) }}" 
                    method="POST" 
                    onsubmit="return confirm('Are you sure you want to delete your rating? You can submit a new rating after deletion.')"
                >
                    @csrf
                    @method('DELETE')
                    <button 
                    ...
                    >
                        <i class="fas fa-trash mr-2"></i> Delete and Resubmit Rating
                    </button>
                </form>
            </div>
        </div>
    @elseif(auth()->check())
        <div class="bg-white rounded-2xl shadow-lg p-6 mb-8">
            <h3 class="font-semibold text-gray-900 mb-4 flex items-center text-lg">
                <i class="fas fa-pen mr-2 text-green-600"></i> Rate This Ground
            </h3>
            
            <form action="{{ route('ground-ratings.store', $ground) }}" method="POST" 
    ...
    @else
        <div class="bg-white rounded-2xl shadow-lg p-6 mb-8">
            <h3 class="font-semibold text-gray-900 mb-4 flex items-center text-lg">
                <i class="fas fa-star mr-2 text-yellow-400"></i> Rate This Ground
            </h3>
            <div class="flex flex-col items-center text-center py-8">
                <div class="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mb-4">
                    <i class="fas fa-lock text-gray-400 text-2xl"></i>
                </div>
                <p class="text-gray-600 mb-6">To rate this ground and help other users, please log in to your account.</p>
                ...
                <p class="text-xs text-gray-500 mt-6">
                    <i class="fas fa-info-circle mr-1"></i> Your feedback helps other users find great grounds
                </p>
            </div>
        </div>
    @endif
    ...
    <div class="bg-white rounded-2xl shadow-lg p-6">
        <h3 class="text-xl font-bold text-gray-900 mb-6 flex items-center">
            <i class="fas fa-comments mr-2 text-green-600"></i> User Reviews
        </h3>
                    ...
                    <div class="border-b border-gray-100 last:border-0 pb-6 last:pb-0">
                        <div class="flex items-start justify-between mb-3">
                            ...
                            <div class="flex text-yellow-400">
                                ...
                                @endfor
                                <span class="ml-2 text-gray-600 text-sm font-medium">{{ $rating->rating }}/5</span>
                            </div>
                        </div>
                        @if($rating->comment)
        ...
        @else
            <div class="text-center py-12">
                ...
                <p class="text-gray-500 text-lg">No ratings yet. Be the first to rate this ground!</p>
            </div>
        @endif
    </div>
</div>
